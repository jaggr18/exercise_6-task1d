...
class Employee(object):
    ...
    def __str__(self):
        return "Employee: %s is of type %s" %(self.get_name(), self.get_title())
...
class Manager(Employee):
    ...
    def get_sales_report(self,salesman):
        if bool(salesman.sales) is False:
            raise KeyError("%s does not have any sales yet." % salesman)
        else:
            print(sum(salesman.sales.values()))
